 ...
 * 
 */
package com.hiperium.device.restful.filter;
...
import com.hiperium.device.bo.identity.SessionManagerBO;
...
public class TokenFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) 
		...
		try {
			if(path.contains(DeviceRestfulPath.GATEWAY)) {
				if(!this.sessionManagerBO.isHomeLoggedIn(tokenId)) {
					HttpServletResponse res = (HttpServletResponse) response;
					res.sendError(HttpServletResponse.SC_UNAUTHORIZED);
					return;
				}
			} else if(!this.sessionManagerBO.isUserLoggedIn(tokenId)) {
				HttpServletResponse res = (HttpServletResponse) response;
				res.sendError(HttpServletResponse.SC_UNAUTHORIZED);
				return;
			}
